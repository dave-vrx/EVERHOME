...
type PetState={kind:string;name:string;hunger:number;happy:number;energy:number;clean:number;xp:number;tokens:number;room:string;decor:string[];garden:{seed:string;planted:number}[];collection:string[]};
function PetHome({notify}:{notify:(s:string)=>void}){
 const defaults:PetState={kind:"fox",name:"Nova",hunger:82,happy:76,energy:68,clean:74,xp:0,tokens:120,room:"meadow",decor:["bed"],garden:[],collection:[]};
 const [pet,setPet]=useState<PetState>(()=>{if(typeof window==="undefined")return defaults;try{return{...defaults,...(JSON.parse(localStorage.getItem("everhome-pet")||"null")||{})}}catch{return defaults}}),[tab,setTab]=useState("Care"),[game,setGame]=useState({active:false,score:0,ends:0}),[now,setNow]=useState(Date.now());
 useEffect(()=>{localStorage.setItem("everhome-pet",JSON.stringify(pet))},[pet]);
 useEffect(()=>{const timer=setInterval(()=>{setNow(Date.now());setGame(g=>g.active&&Date.now()>g.ends?{...g,active:false}:g)},1000);return()=>clearInterval(timer)},[]);
 const pets=[['fox','🦊','Fox'],['cat','🐱','Cat'],['dog','🐶','Pup'],['dragon','🐲','Dragon'],['robot','🤖','Bot']],icon=pets.find(p=>p[0]===pet.kind)?.[1]||"🦊",level=Math.floor(pet.xp/100)+1,mood=(pet.hunger+pet.happy+pet.energy+pet.clean)/4;
 const update=(changes:Partial<PetState>,message:string,xp=8)=>{setPet(p=>({...p,...changes,xp:p.xp+xp}));notify(message)};
 const care=(key:"hunger"|"happy"|"energy"|"clean",message:string)=>setPet(p=>({...p,[key]:Math.min(100,p[key]+22),xp:p.xp+10,tokens:p.tokens+3}));
 const rooms:{[key:string]:[string,string]}={meadow:["🌳","Meadow Cottage"],cloud:["☁️","Cloud Loft"],neon:["🌃","Neon Den"],beach:["🏝️","Beach Hut"]};
 const decor=[['bed','🛏️','Cozy bed',40],['lamp','💡','Star lamp',55],['plant','🪴','Happy plant',35],['beanbag','🛋️','Bean bag',65],['poster','🖼️','World poster',50]] as const;
 const buyDecor=(id:string,cost:number)=>{if(pet.decor.includes(id))return;if(pet.tokens<cost){notify("Not enough Pet Tokens");return}setPet(p=>({...p,tokens:p.tokens-cost,decor:[...p.decor,id]}));notify("Decoration added to your room!")};
 const plant=(seed:string)=>{if(pet.garden.length>=3){notify("All garden plots are full");return}setPet(p=>({...p,garden:[...p.garden,{seed,planted:Date.now()}]}));notify("Seed planted — it will grow shortly!")};
 const harvest=(index:number)=>{const crop=pet.garden[index];if(now-crop.planted<30000){notify("That plant is still growing");return}setPet(p=>({...p,tokens:p.tokens+25,xp:p.xp+20,garden:p.garden.filter((_,i)=>i!==index),collection:[...new Set([...p.collection,crop.seed])]}));notify("Harvested! +25 Pet Tokens")};
 const finishGame=()=>{if(game.active)return;const reward=game.score*2;setPet(p=>({...p,tokens:p.tokens+reward,xp:p.xp+game.score*3,happy:Math.min(100,p.happy+game.score)}));notify(`Star Chase complete: +${reward} Pet Tokens`)};
 useEffect(()=>{if(!game.active&&game.ends>0)finishGame()},[game.active]);
 return <><PageTitle over="EVERHOME COMPANIONS" title="A whole little life together" text="Care, decorate, grow, play, collect and take your forever friend into every world."/>
 <div className="petGameBar"><div><i>{icon}</i><span><b>{pet.name}</b><small>Level {level} · {mood>80?"Thriving":mood>55?"Happy":"Needs care"}</small></span></div><div className="petXp"><i style={{width:`${pet.xp%100}%`}}/><span>{pet.xp%100}/100 XP</span></div><b>◈ {pet.tokens} Pet Tokens</b></div>
 <nav className="petTabs">{["Care","Home","Garden","Play","Collection"].map(t=><button key={t} className={tab===t?"active":""} onClick={()=>setTab(t)}>{({Care:"♡",Home:"⌂",Garden:"✿",Play:"▶",Collection:"◆"} as Record<string,string>)[t]} {t}</button>)}</nav>
 {tab==="Care"&&<div className="petHome"><section className={`petStage petRoom-${pet.room}`}><div className="petRoom"><span className="petSprite">{icon}</span><i>{mood>80?"♥":mood>55?"♡":"…"}</i>{pet.decor.map(d=><em className={`petDecor decor-${d}`} key={d}>{decor.find(x=>x[0]===d)?.[1]}</em>)}</div><input aria-label="Pet name" value={pet.name} maxLength={12} onChange={e=>setPet({...pet,name:e.target.value})}/><b>Bond level {level}</b></section><section className="petCare"><h2>Daily care</h2>{([['hunger','Full tummy',pet.hunger],['happy','Happiness',pet.happy],['energy','Energy',pet.energy],['clean','Clean & cozy',pet.clean]] as const).map(([key,label,value])=><label key={key}><span>{label}</span><b>{value}%</b><i><em style={{width:`${value}%`}}/></i></label>)}<div className="petActions"><button onClick={()=>care('hunger','Berry snack enjoyed!')}>🍓 Feed</button><button onClick={()=>care('happy','Playtime made your companion happy!')}>🧶 Play</button><button onClick={()=>care('energy','A cozy nap restored energy!')}>🛏️ Nap</button><button onClick={()=>care('clean','Fresh, fluffy and clean!')}>🛁 Bathe</button></div><h3>Choose companion</h3><div className="petPicker">{pets.map(([id,pIcon,name])=><button className={pet.kind===id?'selected':''} key={id} onClick={()=>update({kind:id},`${name} is now your active companion!`,0)}><i>{pIcon}</i><span>{name}</span></button>)}</div></section></div>}
 {tab==="Home"&&<div className="petFeature"><section><small>MY ROOM</small><h2>{rooms[pet.room][0]} {rooms[pet.room][1]}</h2><div className={`roomPreview room-${pet.room}`}><span>{icon}</span>{pet.decor.map(d=><i key={d}>{decor.find(x=>x[0]===d)?.[1]}</i>)}</div><div className="roomChoices">{Object.entries(rooms).map(([id,[roomIcon,name]])=><button className={pet.room===id?"selected":""} key={id} onClick={()=>update({room:id},`${name} selected`,0)}>{roomIcon}<span>{name}</span></button>)}</div></section><aside><small>DECORATION SHOP</small><h2>Make it feel like home</h2>{decor.map(([id,dIcon,name,cost])=><article key={id}><i>{dIcon}</i><div><b>{name}</b><small>{pet.decor.includes(id)?"In your room":`◈ ${cost}`}</small></div><button disabled={pet.decor.includes(id)} onClick={()=>buyDecor(id,cost)}>{pet.decor.includes(id)?"Owned":"Buy"}</button></article>)}</aside></div>}
 {tab==="Garden"&&<div className="petFeature petGarden"><section><small>GROWING GARDEN</small><h2>Plant treats and surprises</h2><div className="gardenPlots">{[0,1,2].map(i=>{const crop=pet.garden[i],ready=crop&&now-crop.planted>=30000;return <button key={i} className={ready?"ready":""} onClick={()=>crop&&harvest(i)}>{crop?(ready?"✨":"🌱"):"＋"}<span>{crop?(ready?`${crop.seed} ready!`:`Growing ${Math.max(0,30-Math.floor((now-crop.planted)/1000))}s`):"Empty plot"}</span></button>})}</div></section><aside><small>SEED BAG</small><h2>Choose a seed</h2>{[["Moonberry","🫐"],["Sunbloom","🌻"],["Cloudmint","🌿"]].map(([seed,seedIcon])=><button className="seedPacket" key={seed} onClick={()=>plant(seed)}><i>{seedIcon}</i><span><b>{seed}</b><small>Ready in 30 seconds</small></span><em>Plant</em></button>)}</aside></div>}
 {tab==="Play"&&<div className="petFeature petArcade"><section><small>MINIGAME</small><h2>Star Chase</h2><p>Start a ten-second round, then tap the moving star as many times as you can.</p><div className="starGame">{game.active?<button style={{left:`${12+(game.score*37)%72}%`,top:`${14+(game.score*29)%62}%`}} onClick={()=>setGame(g=>({...g,score:g.score+1}))}>⭐</button>:<button className="startGame" onClick={()=>setGame({active:true,score:0,ends:Date.now()+10000})}>{game.ends?`Score ${game.score} · Play again`:`Start game`}</button>}<b>{game.active?`${Math.max(0,Math.ceil((game.ends-now)/1000))}s · ${game.score} stars`:"Win tokens, happiness and XP"}</b></div></section><aside><small>DAILY ACTIVITIES</small><h2>Play together</h2>{[["🎵","Dance party"],["🧩","Puzzle time"],["🎨","Paint together"]].map(([aIcon,name])=><button className="activityCard" key={name} onClick={()=>update({happy:Math.min(100,pet.happy+8)},`${name} complete!`,6)}><i>{aIcon}</i><span>{name}</span><em>+6 XP</em></button>)}</aside></div>}
 {tab==="Collection"&&<div className="petCollection"><small>COMPANION JOURNAL</small><h2>Everything you&apos;ve discovered</h2><div>{pets.map(([id,pIcon,name])=><article className={pet.kind===id?"found":""} key={id}><i>{pIcon}</i><b>{name}</b><small>{pet.kind===id?"Active companion":"Meet in future events"}</small></article>)}{["Moonberry","Sunbloom","Cloudmint"].map((item,i)=><article className={pet.collection.includes(item)?"found":""} key={item}><i>{["🫐","🌻","🌿"][i]}</i><b>{item}</b><small>{pet.collection.includes(item)?"Garden discovery":"Not discovered"}</small></article>)}</div></div>}
 </>
}
